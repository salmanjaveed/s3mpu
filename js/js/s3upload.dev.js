...
function S3MultiUpload(file) {
  ...
  this.uploadXHR = []; // Progress monitoring
...
S3MultiUpload.prototype.uploadParts = function () {
  ...
  while (start < this.file.size) {
    ...
    filePart = this.file.slice(start, end); // this is to prevent push blob with 0Kb
  ...
  $.when.apply(null, promises).then(this.sendAll.bind(this), this.onServerError).done(this.onPrepareCompleted);
};
/**
...
S3MultiUpload.prototype.sendAll = function () {
  ...
  if (length == 1) this.sendToS3(arguments[0], blobs[0], 0);else for (var i = 0; i < length; i++) {
 ...
 * @param  index    - index part integer (base zero)
 */
...
S3MultiUpload.prototype.sendToS3 = function (data, blob, index) {
  ...
  request.upload.onprogress = function (e) {
    if (e.lengthComputable) {
      ...
      if (index == 0 || self.total[0] == self.loaded[0]) self.updateProgress();
    }
  };
...
function AbortPending() {
  ...
  this.sendBackData = null;
  $.post(self.SERVER_LOC, {
    command: "AbortPending",
    sendBackData: self.sendBackData
  }).done(function (data) {
    self.sendBackData = data;
    self.onAbortPendingCompleted(data);
  }).fail(function (jqXHR, textStatus, errorThrown) {
    self.onServerError("AbortPending", jqXHR, textStatus, errorThrown);
  });
}

AbortPending.prototype.onAbortPendingCompleted = function (data) {};
/**
 * setCORS on S3
 */
...
S3MultiUpload.prototype.updateProgress = function () {
  ...
  if (complete) this.completeMultipartUpload();
