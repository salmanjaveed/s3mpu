 ...
 */
  s3upload.onServerError = function (command, jqXHR, textStatus, errorThrown) {
    $("#result").text(
      "Status: Upload failed with server error. " +
        textStatus +
        " " +
        errorThrown +
        " " +
  ...
  s3upload.onProgressChanged = function (uploadedSize, totalSize, speed) {
    ...
    // var progress = parseInt(uploadedSize / upload.TotalFileSize * 100, 10);
...
function unsetCookies() {
  ...
  document.cookie =
    "BucketName=" + "" + ";domainname;path=/;expires=" + expires;
  document.cookie =
    "FolderName=" + "" + ";domainname;path=/;expires=" + expires;
}
...
function alertDiv(message) {
  $("body").before(
    '<div class="alert alert-danger alert-dismissible" id="AlertBox" data-dismiss="alert" role="alert>' +
...
$(document).ready(function () {
  ...
  $("#startUpload").click(function () {
    if (CheckFields()) {
      ...
      // Upload & Process all selected files
  ...
  $("#AbortPending").click(function () {
    if (CheckFields()) {
      $.confirm({
        content:
          " This will remove all pending S3 Uploads including all partial uploads. Confirm Delete?",
        title: "Heads Up",
        buttons: {
          Cancel: {
            btnClass: "btn-red",
          ...
          Proceed: {
            ...
            action: function () {
              ...
              s3AbortPending.onAbortPendingCompleted = function (data) {
                $("#result").text(
                  "Status: Completed Deleting " + data["count"] + " Objects."
                );
              };
            },
          },
  ...
  $("#EnableCORS").click(function () {
    if (CheckFields()) {
      ...
      $("#result").text(
        "Status: CORS Enabled on bucket " + $("#BucketName")[0].value
      );
      s3SetCORS.onServerError = function (
        command,
        jqXHR,
        textStatus,
        errorThrown
