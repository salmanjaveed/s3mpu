...
S3MultiUpload.prototype.uploadParts = function () {
  ...
  for (var i = 0; i < blobs.length; i++) {
    blob = blobs[i];
    promises.push(
      (this.uploadXHR[i] = $.post(this.SERVER_LOC, {
        command: "part",
...
S3MultiUpload.prototype.sendAll = function () {
  ...
  if (length == 1) this.sendToS3(arguments[0], blobs[0], 0);
  else
  ...
  request.upload.onprogress = function (e) {
    if (e.lengthComputable) {
      ...
      if (self.lastUploadedTime[index]) {
        var time_diff =
          (new Date().getTime() - self.lastUploadedTime[index]) / 1000;
        if (time_diff > 0.005) {
          // 5 miliseconds has passed
          var byterate =
            (self.loaded[index] - self.lastUploadedSize[index]) / time_diff;
 ...
 */
S3MultiUpload.prototype.onServerError = function (
  command,
  jqXHR,
  textStatus,
  errorThrown
 ...
 */
S3MultiUpload.prototype.onProgressChanged = function (
  uploadedSize,
  totalSize,
  bitrate
) {};
